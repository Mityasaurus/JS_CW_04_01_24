...
const cocktailBox = document.querySelector('.cocktailBox');
const cocktailItem = document.querySelector('.cocktailItem');
const cocktailData = createCocktailCards(drinks);

cocktailBox.insertAdjacentHTML('beforeend', cocktailData);

cocktailBox
  .querySelectorAll('.cocktailItem')
  .forEach(ci => ci.addEventListener('click', onCocktailItemClick));

function createCocktailCards(drinks) {
  return drinks
    .map(({ strDrink, strDrinkThumb }) => {
      return `<div class="cocktailItem">
    <ul>
    <li>
    ${strDrink}
    <img src="${strDrinkThumb}" alt="cocktail photo" class="cocktailImg">
    </li>
    </ul>
    </div>`;
    })
...
function onCocktailItemClick(e) {
  const nextActiveCard = e.currentTarget;

  if (nextActiveCard.classList.contains('cocktailItem') === false) {
    return;
  }

  const currentActiveCard = document.querySelector('.isActiveCard');
  if (currentActiveCard) {
    currentActiveCard.classList.remove('isActiveCard');
    currentActiveCard
      .querySelector('.cocktailImg')
      .classList.remove('js-isActive');
  }

  nextActiveCard.querySelector('.cocktailImg').classList.add('js-isActive');
  nextActiveCard.classList.add('isActiveCard');
}
